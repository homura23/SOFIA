...
class MertEncoder(BaseEncoder):
    def __init__(
        ...
        super().__init__(name="mert", target_sr=target_sr, channels=1, embed_dim=hidden_size, device=device)
        ...
        if pooling not in {"layer_mean", "last_layer"}:
            raise ValueError(f"Unsupported MERT layer_pooling '{layer_pooling}'. Use 'layer_mean' or 'last_layer'.")
    ...
    def encode(self, audio: torch.Tensor, sample_rate: int) -> torch.Tensor:
        ...
        for i in range(audio.size(0)):
            ...
            if hs is None:
                raise RuntimeError("MERT forward did not return hidden_states.")
