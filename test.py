...
from sofia.train import build_encoders, evaluate, select_head
from sofia.utils.config import load_config, resolve_device


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a trained multi-branch model")
...
def load_models(cfg: Dict, device: torch.device, checkpoint: str):
    ...
    head = select_head(cfg, embedding_dim=int(fusion_cfg["fusion_out"]), num_classes=int(cfg["training"]["num_classes"])).to(device)
...
def main() -> None:
    ...
    print(f"[eval] loss {stats['loss']:.4f} | acc {stats['acc']:.4f}")
